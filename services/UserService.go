...
func (*UserService) AddUsers(stream pb.UserService_AddUsersServer) error{
	...
	for{
		...
		users = append(users, &pb.User{Id: req.GetId(), Name: req.GetName(), Email: req.GetEmail()})

		log.Print("Adding ", req.GetName())
	}
}
...
func (*UserService) AddUsersStreamBoth(stream pb.UserService_AddUsersStreamBothServer) error{
	for{
		...
		if err != nil{
			log.Fatalf("Error Recieving stream from service: %v", err)
		}
		...
		if err != nil{
			log.Fatalf("Error Send stream to client: %v", err)
		}
